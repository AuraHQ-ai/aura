...
// ── Action Log (append-only audit trail for every tool invocation) ──────────
...
// ── Approval Policies (admin-managed rules for tool governance) ─────────────
...
export const approvalPolicies = pgTable(
  ...
  (table) => [
    index("approval_policies_tool_pattern_idx").on(table.toolPattern),
    index("approval_policies_url_pattern_idx").on(table.urlPattern),
    check(
