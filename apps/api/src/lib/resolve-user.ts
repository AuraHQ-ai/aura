...
export async function resolveSlackUserId(
  ...
  // Fallback: query users in DB (exact match first, then prefix)
  try {
    const { db } = await import("../db/client.js");
    const { users } = await import("@aura/db/schema");
    ...
    const [exact] = await db
      ...
      .from(users)
      .where(ilike(users.displayName, escaped))
      .limit(1);
    ...
    const [prefix] = await db
      ...
      .from(users)
      .where(ilike(users.displayName, `${escaped}%`))
      .orderBy(sql`${users.interactionCount} DESC NULLS LAST`)
      .limit(1);
