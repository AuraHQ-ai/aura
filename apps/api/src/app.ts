...
app.post("/api/slack/events", async (c) => {
  ...
  if (body.event) {
    ...
    if (event.type === "agent_session_stopped") {
      const stopPromise = (async () => {
        ...
        try {
          ...
          const { displaced, stopId } = await stopInvocation(
            channelId,
            threadTs,
            eventTs,
            workspaceId,
      ...
      if (event.tab === "messages") {
        const bootstrapPromise = (async () => {
          try {
            ...
            recordError("app_home_opened_bootstrap", err, { userId: event.user });
          }
        })();
        waitUntil(bootstrapPromise);
      }
    ...
    // Run pipeline asynchronously inside execution context.
