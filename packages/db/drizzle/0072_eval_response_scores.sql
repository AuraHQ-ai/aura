CREATE TABLE IF NOT EXISTS "eval_response_scores" (
	...
	CONSTRAINT "ers_verdict_check" CHECK ("verdict" IS NULL OR "verdict" IN ('fulfilled', 'partial', 'failed')),
	CONSTRAINT "ers_failure_class_check" CHECK ("failure_class" IN ('missing_cred', 'bad_memory', 'bad_harness', 'missing_tool', 'reasoning', 'latency', 'none'))
);
...
END $$;
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "eval_response_scores_workspace_message_idx" ON "eval_response_scores" USING btree ("workspace_id","message_id");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "eval_response_scores_part_idx" ON "eval_response_scores" USING btree ("part_id");--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "eval_response_scores_thread_ts_idx" ON "eval_response_scores" USING btree ("thread_ts");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "eval_response_scores_verdict_idx" ON "eval_response_scores" USING btree ("verdict","failure_class");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "eval_response_scores_created_at_idx" ON "eval_response_scores" USING btree ("created_at");
