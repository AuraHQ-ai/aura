...
While Aura reasons, an agent-session status is shown via `agents.sessions.setStatus` (e.g. "Thinking...", "Thinking deeply..."). Channels where the workspace lacks permission are cached in-process on first failure and skipped on subsequent calls to avoid log spam and repeated retries.
