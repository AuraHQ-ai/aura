...
By default Aura uses `realadvisor/aura-tools`. To point at a different repository, configure the workspace setting `tools_repo` from App Home → Settings → **Self-authored tools repository**. The value is a single GitHub repository in either shorthand or URL form:
...
On every sandbox acquisition, Aura ensures the repository (the `tools_repo` setting, or `realadvisor/aura-tools` when unset) is cloned and up to date at:

```bash
/home/user/aura-tools
```

The checkout is refreshed with `git pull --ff-only` when the directory already contains a valid Git repository, and Python dependencies from `requirements.txt` are installed whenever the checkout changes. A freshness marker (30-minute TTL) makes repeated acquisitions a single cheap probe. Clone, pull, or dependency-install failures are warnings only; they do not make the sandbox unusable.
