...
- `update_email_threads` — Batch-update triage states for multiple threads at once (internal state only; does not touch Gmail).
- `archive_emails` — Archive threads in the real Gmail mailbox (removes the `INBOX` label via `messages.batchModify`; messages stay in All Mail). Also marks archived threads `resolved` locally, and returns per-thread `archived` / `failed` / `not_found` results.
