...
| `min_interval_hours` | number | no | Minimum hours between executions. Anchored to the job's *scheduled* fire time, not wall-clock execution time -- an off-schedule run (e.g. a manual trigger or recovery re-run) never pushes back the next scheduled fire (#1238). |
...
| `env_allowlist` | string[] | no | Restrict the job's sandbox env AND credential-gated typed tools to only these credential env var names (plus core infra vars). Omit for full inheritance. |
| `prompt_mode` | enum | **yes** | `task` (mechanical/fixed-playbook) or `full` (Aura's voice, language nuance, or self-reflection). Required at creation — there is no silent default. See [Prompt Modes](/prompt-modes) |
...
Three fields on `create_job` / `update_job` narrow what a job runs with. `model` and `env_allowlist` are optional; `prompt_mode` is **required on `create_job`** (still optional on `update_job`, which is a partial update). All three are independent and can be combined.
...
### `env_allowlist` — credential narrowing

Restricts the job's credentials to only the listed credential env var names, plus core infra vars that the sandbox needs to function (`E2B_API_KEY`, `E2B_TEMPLATE_ID`, `GOOGLE_SA_KEY_B64`). Matching is case-insensitive, and `GITHUB_TOKEN`/`GH_TOKEN` are aliased to each other.

The allowlist **narrows, never widens**: it intersects with the caller's credential set, so names in the allowlist that the caller can't access are not added. It covers every credential-resolution path in the run:

- **Sandbox env** — every `run_command` in the job and the `script` layer only see allowlisted env vars.
- **Typed tools** — tools gated on a backing credential (`bq_execute_query`, Gmail tools, `web_search`, …) are omitted from the job's tool set when that credential is not in the allowlist. Matching uses the credential's `sandbox_env_name` when set, otherwise the uppercased credential name (e.g. `google_oauth` → `GOOGLE_OAUTH`). This includes tools built at runtime (e.g. inside subagents), so a scoped job can't reach an out-of-allowlist credential through a typed tool.
...
### Required credentials are owner-scoped

Credential resolution for a job run is deliberately scoped to the job owner (`requested_by`) — dispatching a job or transferring it never widens scope. When a job lists required credential IDs and one of them is not available to the owner (for example an admin-scoped credential on a job now owned by a non-admin, or a credential that was deleted), the run **fails closed before executing anything**: the execution is marked failed and an ops notice naming the credential and the job owner goes out via the ops routing ladder. Fix it by granting the credential to the owner or transferring the job to a user with access.
...
Memory stays unified either way: task-mode executions write to the same memory and message store as every other invocation. **`create_job` requires `prompt_mode`** — omitting it used to leave the column NULL, which silently resolved to the full prompt. Rule of thumb: mechanical/fixed-playbook work → `task`; only jobs whose output carries Aura's voice, language nuance, or self-reflection need `full`. On `update_job`, omit the field to leave the existing value unchanged, or set `null` to reset to `full` (the execute-time fallback for NULL).
