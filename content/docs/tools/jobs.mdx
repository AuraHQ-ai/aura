...
The **heartbeat** runs every 30 minutes and evaluates all pending jobs. Scheduling granularity is approximately 30 minutes. Failed jobs retry 3x with 30-minute backoff, then escalate via DM. Jobs without an output channel are intentionally prevented from posting to Slack; their deliverable must be a non-channel side effect such as a note, database write, or internal dispatch.
