...
When memory extraction or schema behavior changes, operators can rebuild memories from the stored conversation history with `pnpm --filter aura-api rebuild:memories` (or the equivalent `scripts/rebuild-memories.ts` entry point). The rebuild pipeline replays eligible threads chronologically, writes provenance-aware memories, and regenerates entity summaries. Review the script help and deployment runbook before running it against production data; it is an administrative maintenance operation, not a normal chat tool.
