...
Pull requests and pushes to `main` run `.github/workflows/ci.yml`. The `ci` job installs with the frozen pnpm lockfile, builds `@aura/db`, runs `pnpm typecheck`, runs the API test suite, and finishes with the Vercel-preset build (`pnpm --filter aura-api build:vercel`). The Vercel build is deliberately the final gate because it exercises the production bundling path.
