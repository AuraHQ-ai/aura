...
| `GITHUB_WEBHOOK_SECRET` | Secret for verifying `X-Hub-Signature-256` on the GitHub webhook endpoint (PR/issue events). Required for that endpoint to accept deliveries -- without it, webhook requests are rejected. |
...
| `CURSOR_DEFAULT_MODEL` | Optional default Cursor model for `dispatch_cursor_agent` when no model is supplied |
