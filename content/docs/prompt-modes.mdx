...
- **Slack tools**: `create_job` **requires** `prompt_mode: "full" | "task"` — there is no silent default. `update_job` still accepts it as an optional partial field; set it to `null` to reset to the execute-time fallback (`full`).
...
Use `full` only when the job writes user-facing prose in Aura's voice, or depends on the notes index (skills, playbooks, org knowledge) to do its work. Mechanical/fixed-playbook work should not run on `full`.

`create_job` makes you pick: there is no omit-and-default path. Rows created outside the tool (or with `update_job` setting `null`) still resolve NULL → `full` at execute time as a defensive fallback.

Prompt mode composes with the other scoped-execution fields on a job: `model` (route to a cheaper catalog category — see [Jobs & Scheduling](/tools/jobs)) and `env_allowlist` (narrow the sandbox env and credential-gated typed tools).
