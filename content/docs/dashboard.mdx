...
- Every assistant response is scored overnight by a fast-tier judge (cron `/api/cron/eval-responses`, 2 AM UTC) using 20-turn sliding windows over each thread
...
Verdict badges also appear inline on assistant messages in the Conversations timeline, linking to the corresponding eval detail page.
